-- 008: Carbon Price Relief tables, in the base lineage.
--
...
--     lineage used UUID);
--   - RLS is enabled with the base lineage's policy helper, and without the
--     "OR current_tenant_id() IS NULL" bypass: migrations and the service role
--     bypass RLS already, and a request with no tenant must see nothing.
--
-- Reference rates are placeholders for testing, as in the original; replace
-- them from the HMRC CDRM table before relying on them.
...
-- ===========================================================================
-- 4. Row-level security on claims
-- ===========================================================================

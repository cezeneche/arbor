...
export function buildRecordTrends(
  ...
  const quarters: QuarterCoverage[] = sortedQuarterKeys.map(key => {
    ...
    const domainCoverage: QuarterDomainCoverage[] = [...domains.entries()].map(([domain, d]) => {
      // Held only to the document types actually submitted, never to every
      // type the domain could contain.
      const expected = expectedFieldsFor([...d.docTypes], compulsoryByDocumentType)

...
describe('runSerializable', () => {
  it('returns the result on first success', async () => {
    const client = { $transaction: jest.fn(async (fn: any) => fn({})) }
    const result = await runSerializable(async () => 'ok', { client: client as any, sleep: noSleep })
  ...
  it('rethrows the conflict after exhausting retries', async () => {
    ...
    await expect(
      runSerializable(async () => 'never', { client: client as any, sleep: noSleep, retries: 3 }),

...
Usage:
    ...
    slack_url = get_secret("SLACK_WEBHOOK_URL", required=False)
...
_logger = logging.getLogger("narrative.secrets")
...
def get_secret(name: str, required: bool = True) -> str:
    ...
    ----------
    name:
        The secret key name (same as the env var name, e.g. "SLACK_WEBHOOK_URL").
    required:

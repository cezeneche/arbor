...
def gate(packet: dict, narrative_json: dict) -> dict:
    ...
    if local_issues:
        return {"approved": False, "issues": local_issues, "status": "rejected"}
    ...
    genai, GenerateContentConfig = _load_google_genai()
    ...
    return {"approved": approved, "issues": normalised, "status": "approved" if approved else "rejected"}
